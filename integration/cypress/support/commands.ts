...
Cypress.Commands.add('getJobJson', () => {
  cy.fixture('job').then((job) => {
    const address = Cypress.env('ORACLE_CONTRACT_ADDRESS')
    job.initiators[0].params.address = address
...
Cypress.Commands.add(
  'login',
  (email = Cypress.env('CHAINLINK_EMAIL'), password = Cypress.env('CHAINLINK_PASSWORD')) => {
    const url = Cypress.env('CHAINLINK_URL')
    cy.visit(url)
